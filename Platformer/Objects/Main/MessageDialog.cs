...
namespace Platformer.Objects.Main
{
    public class MessageDialog : MessageBox
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (option == 0 && kRightPressed)
                option = 1;

            if (option == 1 && kLeftPressed)
                option = 0;
        }
        ...
        public override void DrawActionIcons(SpriteBatch sb)
        {
            ...
            if (option == 0) sb.Draw(Texture, yesPos, new Rectangle(5 * T, 3 * T, 2 * T, T), Color.White, 0, Vector2.Zero, Vector2.One, SpriteEffects.None, Depth);
            ...
            if (option == 1) sb.Draw(Texture, noPos, new Rectangle(5 * T, 3 * T, 2 * T, T), Color.White, 0, Vector2.Zero, Vector2.One, SpriteEffects.None, Depth);

            // cursor
            var curPos = new Vector2(-T + (float)Math.Sin(sin) * 2, 0);
            sb.Draw(Texture, option == 0 ? yesPos + curPos : noPos + curPos, new Rectangle(7 * T, 3 * T, 2 * T, T), Color.White, 0, Vector2.Zero, Vector2.One, SpriteEffects.None, Depth + .001f);
